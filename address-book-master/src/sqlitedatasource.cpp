...
void SQLiteDataSource::createTable()
{
    ...
    std::string sqlStr = "CREATE TABLE IF NOT EXISTS contact1"
...
void SQLiteDataSource::fillContactFromRow(sqlite3_stmt *s, Contact& c)
{
    ...
    c.phoneNumber = reinterpret_cast<const char*>(sqlite3_column_text(s, 4));
...
ErrorInfo SQLiteDataSource::getContact(Contact::ContactId id, Contact& c)
{
    ...
    std::string sqlStr = "SELECT * FROM contact1 WHERE id=?;";
...
{
    //create sql prepared statement
    std::string sqlStr = "SELECT * FROM contact1;";
...
{
    //create sql prepared statement
    std::string sqlStr = "INSERT INTO contact1 VALUES("
                        "NULL,?,?,?,?,?,?);";
    ...
    sqlite3_bind_text(insertStatement.get(), 4, c.phoneNumber.c_str(), -1, SQLITE_STATIC);
...
{
    //create sql prepared statement
    std::string sqlStr = "UPDATE contact1 SET "
                         "firstName=?, lastName=?,"
                         "address=?, phoneNumber=?,"
                         "email=? , city=? WHERE id=?;";
    ...
    sqlite3_bind_text(updateStatement.get(), 4, c.phoneNumber.c_str(), -1, SQLITE_STATIC);
...
ErrorInfo SQLiteDataSource::deleteContact(Contact::ContactId id)
{
    ...
    //create sql prepared statement
    std::string sqlStr = "DELETE FROM contact1 WHERE id=?;";
...
{
    //create sql prepared statement
    std::string sqlStr = "DELETE FROM contact1;";
